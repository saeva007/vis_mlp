...
# ==========================================
# 0. Global config: VERA-inspired Stage 1 experiment
# ==========================================
TARGET_WINDOW_SIZE = 12
BASE_PATH = "/public/home/putianshu/vis_mlp"
# Dataset produced by s1_data_aerosol_vera.py. Layout is inferred from X_train.npy.
S1_DIR = f"/public/home/putianshu/vis_mlp/ml_dataset_pmst_v5_aligned_{TARGET_WINDOW_SIZE}h_pm10_pm25_vera"

CONFIG = {
    # ========== 实验控制 ==========
    'EXPERIMENT_ID':          os.environ.get('EXPERIMENT_JOB_ID', os.environ.get('S1_EXPERIMENT_ID', 'exp_vera')),
    'S1_PRETRAINED_PATH':     None,
    'BASE_PATH':              BASE_PATH,
    'WINDOW_SIZE':            TARGET_WINDOW_SIZE,
    'S1_DATA_DIR':            S1_DIR,
    # Cluster-stable default: keep a small amount of input parallelism so ranks do
    # not starve on CPU-side preprocessing, but avoid the large worker fan-out
    # that previously triggered NFS/worker instability on big jobs. Override via
    # PMST_NUM_WORKERS=0/1/2 at launch time if a node group behaves differently.
    'NUM_WORKERS':            int(os.environ.get('PMST_NUM_WORKERS', '1')),
    'PREFETCH_FACTOR':        int(os.environ.get('PMST_PREFETCH_FACTOR', '1')),
    'DATALOADER_TIMEOUT_SEC': int(os.environ.get('PMST_DATALOADER_TIMEOUT_SEC', '900')),
    'S1_LOG_INTERVAL':        int(os.environ.get('PMST_LOG_INTERVAL', '10')),
    'FETCH_WARN_SEC':         float(os.environ.get('PMST_FETCH_WARN_SEC', '120')),
    ...
    # Per-step dynamic vars and FE dims are defaults; load_data resolves both from X_train.npy.
    'DYN_VARS_COUNT':          27,
    # FE = 32 legacy fog proxies + 16 VERA-style intermediates + 4 time encodings.
    'FE_EXTRA_DIMS':          52,
    'PHYSICS_FEAT_DIM':       10,
    'GRAD_CLIP_NORM':         0.5,
    'REG_LOSS_ALPHA':         0.1,
    # 已弃用：验证集固定为数据目录中的 X_val.npy / y_val.npy
    'VAL_SPLIT_RATIO':        0.1,
...
def safe_barrier(world_size: int, device: torch.device = None):
    ...
    与 PMST_net_test_10_s1.py 一致：在当前 GPU 上做 dummy all_reduce + cuda.synchronize，
    保证集体通信与 GPU 完成顺序一致（比 TCPStore-only barrier 更简单、更稳）。
    """
...
class ImprovedDualStreamPMSTNet(nn.Module):
    def __init__(self, dyn_vars_count=25, window_size=12, static_cont_dim=5,
        ...
        self.temporal_var_indices = self._build_temporal_indices()
    ...
    def _build_temporal_indices(self):
        base = [0, 1, 2, 4, 6, 10, 22, 23, 12, 11, 13, 15, 14]
        if self.dyn_vars >= 27:
            base.extend([self.dyn_vars - 2, self.dyn_vars - 1])
        elif self.dyn_vars >= 26:
            base.append(self.dyn_vars - 1)
        seen, out = set(), []
        for i in base:
            if 0 <= i < self.dyn_vars and i not in seen:
                out.append(i)
                seen.add(i)
        return out
    ...
    def _physics_features(self, x):
        ...
        dpd   = x[:, :, 22]
        inv   = x[:, :, 23]
        lcc   = torch.clamp(x[:, :, 10], 0.0, 1.0)

        if self.dyn_vars >= 27:
            pm10 = torch.clamp(x[:, :, self.dyn_vars - 2], min=0.0)
            pm25 = torch.clamp(x[:, :, self.dyn_vars - 1], min=0.0)
        elif self.dyn_vars >= 26:
            pm10 = torch.clamp(x[:, :, self.dyn_vars - 1], min=0.0)
            pm25 = torch.zeros_like(pm10)
        else:
            pm10 = torch.zeros_like(rh2m)
            pm25 = torch.zeros_like(rh2m)

        rh_frac = torch.clamp(rh2m / 100.0, 0.01, 0.995)
        one_minus_rh = torch.clamp(1.0 - rh_frac, min=0.005)
        growth = torch.clamp(torch.pow(one_minus_rh, -0.85), 1.0, 10.0)
        coarse = torch.clamp(pm10 - pm25, min=0.0)
        hydrated_fine = torch.log1p(pm25) * torch.log1p(growth)
        hydrated_coarse = torch.log1p(coarse) * torch.log1p(torch.sqrt(growth))
        aerosol_ext = 0.80 * hydrated_fine + 0.25 * hydrated_coarse
        liquid_proxy = torch.sigmoid((rh2m - 95.0) / 2.5) * torch.sigmoid(-dpd / 1.5) * (0.5 + 0.5 * lcc)
        rh_error_sens = torch.log1p(torch.clamp((rh_frac / one_minus_rh) * growth, 0.0, 80.0))
        ...
        f8 = torch.log1p(aerosol_ext)
        f9 = liquid_proxy
        f10 = rh_error_sens

        return torch.stack([f1, f2, f3, f4, f5, f6, f7, f8, f9, f10], dim=2).mean(dim=1)
...
def _resolve_dyn_and_fe_dims(total_dim: int, win_size: int):
    """
    X 布局：WINDOW * dyn_vars + 5(static) + 1(veg) + FE_EXTRA。
    在 dyn 候选 (27,26,25,24) 中选取使 FE 落在合理区间者，避免 PM2.5 误算进 FE。
    """
    ...
    for dyn in (27, 26, 25, 24):
        ...
        if 20 <= fe <= 64:
            return dyn, fe
    raise ValueError(
        f"Cannot resolve dyn/FE: total_dim={total_dim}, WINDOW={win_size}, "
        f"rest={rest}. Rebuild data or check layout."
    )


def _dyn_indices_log1p(dyn_vars_count: int):
    """与数据管线一致：降水/辐射/CAPE + 气溶胶(末1～2 通道) 做 log1p。"""
    idxs = [2, 4, 9]
    if dyn_vars_count >= 27:
        idxs.extend([dyn_vars_count - 2, dyn_vars_count - 1])
    elif dyn_vars_count >= 26:
        idxs.append(dyn_vars_count - 1)
    return idxs


def _build_y_cls_raw(y_arr: np.ndarray):
    y_raw = np.asarray(y_arr, dtype=np.float32).copy()
    y_cls = np.zeros(len(y_raw), dtype=np.int64)
    if len(y_raw) and np.max(y_raw) < 100:
        y_raw = y_raw * 1000.0
    y_cls[y_raw >= 500] = 1
    y_cls[y_raw >= 1000] = 2
    return y_raw, y_cls
...
def load_data(data_dir, scaler=None, rank=0, local_rank=0, device=None,
    ...
    """
    训练/验证使用 s1_data_aerosol_vera.py 生成的 X_train/y_train + X_val/y_val。
    RobustScaler 仅在训练集行上拟合，再用于 train与 val，避免验证信息泄漏。
    不再对 X_train 做随机子集划分（原做法会把同一站相邻时间窗随机拆到 train/val，造成时间泄漏）。
    """
    path_X_tr = os.path.join(data_dir, 'X_train.npy')
    path_y_tr = os.path.join(data_dir, 'y_train.npy')
    path_X_va = os.path.join(data_dir, 'X_val.npy')
    path_y_va = os.path.join(data_dir, 'y_val.npy')
    if not os.path.isfile(path_X_tr) or not os.path.isfile(path_y_tr):
        raise FileNotFoundError(f"Missing X_train.npy or y_train.npy under {data_dir}")
    if not os.path.isfile(path_X_va) or not os.path.isfile(path_y_va):
        raise FileNotFoundError(
            f"Missing X_val.npy or y_val.npy under {data_dir}. "
            "Run s1_data_aerosol_vera.py so the VERA feature layout and val split are available."
        )

    path_X_tr = copy_to_local(path_X_tr, rank, local_rank, world_size, exp_id)
    path_y_tr = copy_to_local(path_y_tr, rank, local_rank, world_size, exp_id)
    path_X_va = copy_to_local(path_X_va, rank, local_rank, world_size, exp_id)
    path_y_va = copy_to_local(path_y_va, rank, local_rank, world_size, exp_id)
    ...
    if X_va_shape[1] != X_m_shape[1]:
        raise ValueError(
            f"X_val.npy feature dim {X_va_shape[1]} != X_train.npy {X_m_shape[1]}"
        )
    ...
    if rank == 0 and int(CONFIG.get('DYN_VARS_COUNT', dyn_vars_count)) != dyn_vars_count:
        print(
            ...
            f"-> resolved from X_train.npy = {dyn_vars_count} "
            f"(FE_EXTRA={inferred_extra_dim}).",
            flush=True,
        )
    ...
    if len(y_raw_tr) != X_m_shape[0] or len(y_raw_va) != X_va_shape[0]:
        raise ValueError(
            f"y length mismatch: train {len(y_raw_tr)} vs X_train {X_m_shape[0]}, "
            f"val {len(y_raw_va)} vs X_val {X_va_shape[0]}"
        )

    # 与无 pm10 的 S1 区分开，避免混用 scaler
    scaler_path = os.path.join(
        CONFIG['SAVE_CKPT_DIR'],
        f'robust_scaler_w{win_size}_dyn{dyn_vars_count}_s1_vera.pkl'
    )
    ...
    if rank == 0:
        print(
            f"[Data] Train samples={len(y_raw_tr)}, Val samples={len(y_raw_va)} "
            f"(from X_train/X_val files; no random split).",
            flush=True,
        )
...
class ComprehensiveMetrics:
    ...
    def evaluate(self, model, loader, device, rank=0, world_size=1, actual_val_size=None):
        ...
        # 各 rank 的 val DataLoader 步数必须一致，否则先结束循环的 rank 会在 all_reduce 处永久等待。
        if world_size > 1:
            torch.cuda.synchronize(device)
            ...
            if min_b.item() != max_b.item():
                raise RuntimeError(
                    ...
                    f"Check DistributedSampler / batch_size / dataset."
                )

        # 注意：DDP.join() 面向训练（依赖 autograd），推理 + no_grad 下不适用；此处用 len(loader) 全 rank 对齐检查代替。
        with torch.no_grad():
...
def train_stage(tag, model, tr_ds, val_ds, optimizer, loss_fn, device,
    ...
    num_workers = max(0, int(CONFIG.get('NUM_WORKERS', 1)))
    persistent = (num_workers > 0)
    prefetch_factor = max(1, int(CONFIG.get('PREFETCH_FACTOR', 1)))
    dataloader_timeout = max(0, int(CONFIG.get('DATALOADER_TIMEOUT_SEC', 900)))
    ...
    if num_workers > 0:
        val_loader_kwargs['persistent_workers'] = persistent
        val_loader_kwargs['prefetch_factor'] = prefetch_factor
        val_loader_kwargs['timeout'] = dataloader_timeout
    ...
    history_path = os.path.join(CONFIG['SAVE_CKPT_DIR'], f"{exp_id}_{tag}_history.json")
    history = {
    ...
    if rank == 0:
        ...
        print(f"[{tag}] DataLoader config: num_workers={num_workers}, "
              f"prefetch_factor={(prefetch_factor if num_workers > 0 else 'off')}, "
              f"timeout={(dataloader_timeout if num_workers > 0 else 0)}s", flush=True)
        print(f"[{tag}] Next: RCCL warmup (all_reduce). If no 'RCCL warmup passed' in 5-10 min, "
              f"first collective is hanging (try 2 nodes or NCCL_ASYNC_ERROR_HANDLING=1).", flush=True)

    # RCCL warmup: 在训练循环前做一次 all_reduce，避免"首步 backward 才挂"难以定位。
    if world_size > 1:
    ...
    log_interval = max(1, int(CONFIG.get('S1_LOG_INTERVAL', 10)))
    fetch_warn_sec = float(CONFIG.get('FETCH_WARN_SEC', 120.0))
    recent_fetch_s = 0.0
    recent_h2d_s   = 0.0
    recent_fwd_s   = 0.0
    recent_bwd_s   = 0.0
    ...
    while step < total_steps:
        ...
        if rank == 0 and fetch_s >= fetch_warn_sec:
            print(f"[{tag}] WARNING: data fetch took {fetch_s:.1f}s "
                  f"(step={step}, batch={batch_count + 1}, pseudo_epoch={pseudo_epoch}).",
                  flush=True)
        ...
        if rank == 0 and not first_batch_logged:
            print(f"[{tag}] First batch received. fetch={fetch_s:.2f}s, h2d={h2d_s:.2f}s",
                  flush=True)
        ...
        if is_last_accum_step:
            ...
            optimizer.zero_grad()
            opt_s = time.perf_counter() - opt_t0
            step += 1
            ...
            if rank == 0 and (step <= 5 or step % log_interval == 0):
                print(
                    f"[{tag}] Step {step:>6}/{total_steps} | "
                    ...
                    f"t(fetch/h2d/fwd/bwd/opt)="
                    f"{recent_fetch_s / max(recent_batches, 1):.2f}/"
                    f"{recent_h2d_s / max(recent_batches, 1):.2f}/"
                    f"{recent_fwd_s / max(recent_batches, 1):.2f}/"
                    f"{recent_bwd_s / max(recent_batches, 1):.2f}/"
                    f"{recent_opt_s / max(recent_steps, 1):.2f}s | "
                    f"pseudo_epoch={pseudo_epoch}",
                    flush=True
                )
                recent_fetch_s = 0.0
                recent_h2d_s   = 0.0
