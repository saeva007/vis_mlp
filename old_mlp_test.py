...
import warnings
import time
import hashlib  # 新增：脚本1 copy_to_local 需要
...
def load_and_preprocess_balanced_data(path, batch_size=64, threshold1=500, threshold2=1000,
                                      rank=0, device=None, world_size=1, exp_id="default_exp",
                                      data_path=None):
    """
    改进的数据加载函数 - 使用mmap和采样器
    data_path: 实际数据目录（若为None则使用原默认路径）
    """
    ...
    X_train_path = copy_to_local(raw_X_train_path, local_rank, world_size, exp_id)
    X_val_path   = copy_to_local(raw_X_val_path,   local_rank, world_size, exp_id)
    y_train_path = copy_to_local(raw_y_train_path, local_rank, world_size, exp_id)
    y_val_path   = copy_to_local(raw_y_val_path,   local_rank, world_size, exp_id)
...
def main():
    ...
    # ★ 新增：两阶段数据路径 ★
    s1_data_path  = "/public/home/putianshu/vis_mlp/ml_dataset_pmst_v5_aligned_12h"  # 预训练数据
    s2_data_path  = "/public/home/putianshu/vis_mlp/ml_dataset_fe_12h"               # 微调数据
    skip_pretrain = False  # 设为 True 可跳过预训练阶段
    ...
    if not skip_pretrain:
        ...
        train_ds_s1, val_ds_s1, scaler_s1, class_weights_s1 = load_and_preprocess_balanced_data(
            base_path, batch_size, threshold1, threshold2,
            global_rank, device, world_size, exp_id,
            data_path=s1_data_path
        )
        ...
        criterion_s1 = ProgressiveBalancedLoss(
            class_weights=class_weights_s1.to(device),
            total_epochs=pretrain_epochs
        )

        optimizer_s1  = optim.AdamW(model.parameters(), lr=base_learning_rate, weight_decay=2e-5)
        scheduler_s1  = CosineAnnealingWarmRestarts(optimizer_s1, T_0=150, T_mult=2, eta_min=5e-6)
        scaler_amp_s1 = GradScaler()

        sampler_s1 = InfiniteBalancedSampler(
            train_ds_s1,
            batch_size=batch_size,
            pos_ratio=pos_ratio,
            rank=global_rank,
        ...
        train_loader_s1  = DataLoader(
            train_ds_s1,
            batch_sampler=sampler_s1,
            num_workers=4,
            pin_memory=True
        )
        train_iter_s1    = iter(train_loader_s1)
        steps_per_epoch  = 1000

        if global_rank == 0:
            print(f"开始预训练（{pretrain_epochs} epochs）...\n")
        ...
        for epoch in range(pretrain_epochs):
            ...
            for step in range(steps_per_epoch):
                try:
                    batch_x, batch_y, _, _ = next(train_iter_s1)
                ...
                with autocast():
                    ...
                    loss = criterion_s1(main_logits, batch_y, epoch)
                ...
                scaler_amp_s1.scale(loss).backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
                scaler_amp_s1.step(optimizer_s1)
                scaler_amp_s1.update()
            ...
            if global_rank == 0 and (epoch + 1) % 50 == 0:
                train_acc = 100 * correct / total
                avg_loss  = total_loss / steps_per_epoch
                print(f"[Pretrain] Epoch {epoch+1}/{pretrain_epochs} | "
                      f"Loss={avg_loss:.4f} | TrainAcc={train_acc:.2f}%", flush=True)

        # 释放预训练数据资源
        del train_ds_s1, val_ds_s1, train_loader_s1, val_loader_s1
        del optimizer_s1, scheduler_s1, scaler_amp_s1, criterion_s1
        gc.collect()
    ...
    # ==========================================
    # Stage 2：微调阶段（原主训练逻辑，完全不变）
    # ==========================================
    if global_rank == 0:
        print("="*70)
        print("  === Stage 2: 微调（主训练） ===")
        print("="*70)

    # 加载数据（指向 s2_data_path，与原逻辑一致）
    train_ds, val_ds, scaler, class_weights = load_and_preprocess_balanced_data(
        base_path, batch_size, threshold1, threshold2,
        global_rank, device, world_size, exp_id,
        data_path=s2_data_path
    )
    ...
    # 训练循环
    best_score         = 0.0
    # ★ 新增：最佳fog召回率和最佳fog精确率追踪 ★
    best_fog_recall    = 0.0
    best_fog_precision = 0.0
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 15 == 0:
            ...
            if global_rank == 0:
                ...
                # fog 精确率（precision_per_class[0]）和 fog 召回率（recall_500）
                fog_precision_val = float(metrics['precision_per_class'][0]) if len(metrics['precision_per_class']) > 0 else 0.0
                fog_recall_val    = float(metrics['recall_500'])

                print(f"  Fog精确率(class0): {fog_precision_val:.4f}")
                print(f"  Fog召回率(class0): {fog_recall_val:.4f}")
                ...
                # ★ 新增：保存最佳 fog 召回率模型 ★
                if fog_recall_val > best_fog_recall:
                    best_fog_recall = fog_recall_val
                    torch.save({
                        'model_state_dict': model_to_save.state_dict(),
                        'optimizer_state_dict': optimizer.state_dict(),
                        'scheduler_state_dict': scheduler.state_dict(),
                    ...
                    }, os.path.join(base_path, "model/old_mlp_fe_best_fog_recall.pth"))
                    print(f"✨ 保存最佳Fog召回率模型 (fog_recall={fog_recall_val:.4f})")

                # ★ 新增：保存最佳 fog 精确率模型 ★
                if fog_precision_val > best_fog_precision:
                    best_fog_precision = fog_precision_val
                    torch.save({
                        'model_state_dict': model_to_save.state_dict(),
                        'optimizer_state_dict': optimizer.state_dict(),
                        'scheduler_state_dict': scheduler.state_dict(),
                        'epoch': epoch,
                        'metrics': metrics,
                        'fog_precision': fog_precision_val,
                        'config': {
                            'input_dim': input_dim,
                            'hidden_dim': hidden_dim,
                            'num_classes': num_classes,
                    ...
                    }, os.path.join(base_path, "model/old_mlp_fe_best_fog_precision.pth"))
                    print(f"✨ 保存最佳Fog精确率模型 (fog_precision={fog_precision_val:.4f})")
    ...
    if global_rank == 0:
        ...
        print(f"   最佳Fog召回率: {best_fog_recall:.4f}")
        print(f"   最佳Fog精确率: {best_fog_precision:.4f}")
