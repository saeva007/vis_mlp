...
INPUT_FILE = '/public/home/putianshu/vis_mlp/tianji_auto_station/merged_final_all_vars.nc'
VEG_FILE = "/public/home/putianshu/vis_cnn/data_vegtype.nc"
ORO_FILE = "/public/home/putianshu/vis_cnn/data_orography.nc"
OUTPUT_DATASET_DIR = os.path.join(BASE_PATH, 'ml_dataset_fe_12h')

VAR_MAPPING = {
    'rh2m': 'RH2M', 'TMP2m': 'T2M', 'PRATEsfc': 'PRECIP', 'slp': 'MSLP',
    'DSWRFsfc': 'SW_RAD', 'UGRD10m': 'U10', 'VGRD10m': 'V10', 'wind_speed': 'WSPD10',
    'wd10m': 'WDIR10', 'cape': 'CAPE', 'cldl': 'LCC', 't925': 'T_925',
    'rh925': 'RH_925', 'u925': 'U_925', 'v925': 'V_925', 'wind_speed_925': 'WSPD925',
    'dp1000': 'DP_1000', 'dp925': 'DP_925', 'q1000': 'Q_1000', 'q925': 'Q_925',
    'omg925': 'W_925', 'omg1000': 'W_1000', 't1000': 'T_1000'
}
...
GAP_HOURS = 24      # [新增] 各划分之间的时间间隔（小时），防止数据泄漏
...
def compute_fog_features(X_dyn_window, window_size=12, dyn_vars=25):
    ...
    # Summer-regime diagnostics for mixing, convection, and low-level dynamics.
    u10 = X_current[:, 5]
    v10 = X_current[:, 7]
    u925 = X_current[:, 13]
    v925 = X_current[:, 15]
    ...
    daytime_mixing = (
        1.0 / (1.0 + np.exp(-(np.maximum(sw_rad, 0.0) - 150.0) / 75.0))
    ) * (
...
def calculate_zenith_angle(latitudes, longitudes, times):
    try:
        ...
        if times_pd.tz is None: times_pd = times_pd.tz_localize('UTC')
    except:
...
def get_nearest_veg(latitudes, longitudes, veg_data):
    return veg_data.sel(latitude=xr.DataArray(latitudes, dims="station"),
                        longitude=xr.DataArray(longitudes, dims="station"),
                        method="nearest")['htcc'].values
...
def extract_terrain(latitudes, longitudes, oro_ds, r=2):
    ...
    for r_idx, c_idx in zip(lats_idx, lons_idx):
        w = h[max(0, r_idx-r):min(max_r, r_idx+r+1), max(0, c_idx-r):min(max_c, c_idx+r+1)]
        feats.append([h[r_idx, c_idx], h[r_idx, c_idx]-np.mean(w), np.std(w)])
    return np.array(feats, dtype=np.float32)

def prepare_raw_data(ds_in, data_veg, data_oro):
    print("\n--- Processing Data (Memory Optimized) ---")
    ds = ds_in.copy()
    if 'vis' in ds: ds = ds.rename({'vis': 'visibility'})
    ds = ds.rename({k: v for k, v in VAR_MAPPING.items() if k in ds})

    if 'D2M' not in ds: ds['D2M'] = calculate_dewpoint_from_rh(ds['T2M'], ds['RH2M'])
    ds['DPD'] = ds['T2M'] - ds['D2M']
    ds['INVERSION'] = ds['T_925'] - ds['T2M']

    print(f"  Filtering Visibility > {MAX_VIS_THRESHOLD}...")
    ...
    if 'lat' in ds:
        ...
        raise AttributeError("Latitude/Longitude coordinates not found (checked 'lat' and 'latitude').")

    times, stations = pd.to_datetime(ds.time.values), ds.station_id.values

    print("  Building Dynamic Matrix...")
    X_met = ds[FINAL_FEATURE_ORDER].to_array(dim='variable').transpose('time', 'station_id', 'variable').values.astype(np.float32)
    del ds; gc.collect()

    zenith = calculate_zenith_angle(lats, lons, times)
    X_dyn = np.concatenate([X_met, zenith], axis=-1)
    del X_met, zenith; gc.collect()
    ...
    X_stat = np.concatenate([(lats[:, None]/90.0).astype(np.float32),
                             (lons[:, None]/180.0).astype(np.float32),
                             feat_oro, feat_veg], axis=1)
    return X_dyn, X_stat, lats, lons, times, stations

# ================= [新增] 月内划分掩码函数 =================
def get_monthly_split_mask(sample_times):
    """
    在每个自然月内按时间顺序划分 train / val / test，
    相邻分段之间留 GAP_HOURS 小时间隔以防止数据泄漏。
    sample_times: pd.DatetimeIndex，仅包含有效样本的时间戳。
    返回三个布尔数组，形状与 sample_times 相同。
    """
    ...
    for month_period in times.to_period('M').unique():
        ...
        # 各段分割时间点
        split_train = start + pd.Timedelta(seconds=total_sec * TRAIN_RATIO)
        split_val   = start + pd.Timedelta(seconds=total_sec * (TRAIN_RATIO + VAL_RATIO))

        mask_month = (times >= start) & (times <= end)
        train_mask |= mask_month & (times <  split_train)
        val_mask   |= mask_month & (times >= split_train + gap) & (times <  split_val)
        test_mask  |= mask_month & (times >= split_val   + gap)
...
def save_chunked(X_dyn_flat, X_stat_flat, fe_flat, y_flat, mask, meta, out_dir):
    ...
    n = len(valid_idxs)
    print(f"  Valid Samples: {n} ({n/len(mask):.1%})")

    # [修改] 在有效样本内按月内时间顺序划分，替换原来的顺序切分
    ...
    for tag, ix in splits.items():
        ...
        print(f"    Saving {tag} (N={len(ix)})")
        np.save(os.path.join(out_dir, f'y_{tag}.npy'), y_flat[ix])
        pd.DataFrame({'time': times_all[ix], 'station_id': stats_all[ix],
                      'lat': lats_all[ix], 'lon': lons_all[ix]})\
          .to_csv(os.path.join(out_dir, f'meta_{tag}.csv'), index=False)

        fp = np.lib.format.open_memmap(os.path.join(out_dir, f'X_{tag}.npy'), mode='w+', dtype='float32', shape=(len(ix), sum(dims)))

        bs = 100000
        for i in tqdm(range(0, len(ix), bs)):
            bi = ix[i:i+bs]
            n_bi = len(bi)
...
def main():
    ...
    if os.path.isdir(PM10_DIR):
        pm10_files = sorted(glob(os.path.join(PM10_DIR, "*.nc")))
        if pm10_files:
            pm10_list = []
            for fp in pm10_files:
                try:
                    ds_pm10 = xr.open_dataset(fp, engine='h5netcdf')
                    if 'station_id' not in ds_pm10.coords and 'station_id' not in ds_pm10.data_vars:
                        for alias in ['num_station', 'id', 'station']:
                            if alias in ds_pm10.coords or alias in ds_pm10.data_vars:
                                ds_pm10 = ds_pm10.rename({alias: 'station_id'})
                                break
                    var_name = list(ds_pm10.data_vars)[0]
                    pm10_list.append(ds_pm10[var_name])
                except Exception as e:
                    print(f"Warning: skip PM10 file {fp}: {e}")
            if pm10_list:
                pm10_all = xr.concat(pm10_list, dim="time")
                common_time = np.intersect1d(ds_in.time.values, pm10_all.time.values)
                if common_time.size > 0:
                    common_sid = np.intersect1d(ds_in.station_id.values, pm10_all.station_id.values)
                    pm10_sel = pm10_all.sel(time=common_time, station_id=common_sid).astype(np.float32)
                    ds_in = ds_in.sel(time=common_time, station_id=common_sid)
                    ds_in['PM10'] = pm10_sel

    X_dyn, X_stat, lats, lons, times, stats = prepare_raw_data(ds_in, data_veg, data_oro)

    print(f"  Generating Windows ({WINDOW_SIZE})...")
    nt, ns, nv = X_dyn.shape
    n_wins = (nt - WINDOW_SIZE) // STEP_SIZE + 1
    idx = np.arange(WINDOW_SIZE)[None, :] + np.arange(n_wins)[:, None] * STEP_SIZE

    y = ds_in['vis' if 'vis' in ds_in else 'visibility'].values[WINDOW_SIZE-1::STEP_SIZE].reshape(-1)
    ...
    # Transpose to (N_wins, Station, Window, Vars) -> (Samples, Window, Vars)
    X_wins = X_wins.transpose(0, 1, 3, 2)
    X_samples = X_wins.reshape(-1, WINDOW_SIZE, nv)
    del X_wins; gc.collect()

    print("  Computing Feature Engineering...")
    fe_flat = compute_fog_features(X_samples, WINDOW_SIZE, nv)

    print("  Adding Cyclical Time Encoding (+4 features)...")
    ...
    fe_flat = np.concatenate([fe_flat, time_features], axis=1)
    print(f"  FE features: {fe_flat.shape[1]} (32 physics/regime + 4 cyclical)")

    X_dyn_flat = X_samples.reshape(X_samples.shape[0], -1)
    del X_samples, X_dyn; gc.collect()

    print("  Broadcasting Static...")
    X_stat_flat = np.tile(X_stat, (n_wins, 1))

    print("  Saving...")
    mask = ~np.isnan(y) & (y >= 0) & (y <= MAX_VIS_THRESHOLD)
    save_chunked(X_dyn_flat, X_stat_flat, fe_flat, y, mask, (m_t, m_s, m_la, m_lo), OUTPUT_DATASET_DIR)
    print("\nDone!")
