...
case "${RUN_PREFIX}${CACHE_ID}${CKPT_DIR}${BASE}" in
    *","*|*" "*)
        echo "ERROR: BASE, RUN_PREFIX, CACHE_ID, and CKPT_DIR must not contain spaces or commas for sbatch --export." >&2
        exit 2
        ;;
esac
...
for exp_id in ${EXPERIMENT_LIST}; do
    ...
    s1_job="$(
        sbatch --parsable \
            --export=ALL,LOWVIS_RNN_MODE=s1,LOWVIS_RNN_SAMPLING_EXPERIMENTS=${exp_id},LOWVIS_RNN_RUN_PREFIX=${RUN_PREFIX},LOWVIS_RNN_LOCAL_CACHE_ID=${CACHE_ID},LOWVIS_RNN_CKPT_DIR=${CKPT_DIR} \
            "${SBATCH_SCRIPT}"
    )"
    ...
    s2_job="$(
        sbatch --parsable \
            ...
            --export=ALL,LOWVIS_RNN_MODE=s2,LOWVIS_RNN_SAMPLING_EXPERIMENTS=${exp_id},LOWVIS_RNN_RUN_PREFIX=${RUN_PREFIX},LOWVIS_RNN_LOCAL_CACHE_ID=${CACHE_ID},LOWVIS_RNN_CKPT_DIR=${CKPT_DIR},LOWVIS_RNN_PRETRAINED_CKPT=${s1_ckpt} \
            "${SBATCH_SCRIPT}"
    )"
