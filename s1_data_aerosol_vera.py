...
OUTPUT_DATASET_DIR = os.path.join(BASE_PATH, 'ml_dataset_pmst_v5_aligned_12h_pm10_pm25_vera')
...
def process_year_data_optimized(year, ds_obs_full, data_veg, data_oro, pm10_da_year, pm25_da_year=None):
    ...
    for s_idx in tqdm(range(n_stations), desc="Station Loop"):
        ...
        # S1 and S2 must stay synchronized: 32 legacy fog proxies + 16 VERA-style proxies.
        expected_fe_core = len(BASE_FOG_FEATURE_NAMES) + len(VERA_FEATURE_NAMES)
        if fe_features.shape[1] != expected_fe_core:
            raise ValueError(
                f"compute_fog_features dim mismatch: got {fe_features.shape[1]}, expected {expected_fe_core}. "
        ...
        if fe_with_time.shape[1] != len(FE_FEATURE_NAMES):
            raise ValueError(
                f"fe_with_time dim mismatch: got {fe_with_time.shape[1]}, expected {len(FE_FEATURE_NAMES)}."
            )
