...
# 显式保存 init 时创建的 TCPStore，供 safe_barrier 使用，避免依赖 _get_default_store()
# 或首次 dist.barrier() 在 10 节点 RCCL 下锁死（见日志 109621874.out）
_DIST_STORE = None
...
CONFIG = {
    ...
    # Fog/Mist 边界分离 (Priority 4): 显式拉开 logit_fog 与 logit_mist，减轻 Fog/Mist 混淆
    'S2_PAIR_MARGIN':               0.5,    # logit 差至少 margin，否则产生损失
    'S2_LOSS_ALPHA_PAIR_MARGIN':    0.3,

    # Fog/Mist 边界分离 (Priority 4): 在 logit 空间拉开 Fog vs Mist，缓解 500m 边界混淆
    'S2_PAIR_MARGIN':               0.5,    # logit 间隔：希望 logit_fog - logit_mist 与真实类别同向且至少差此值
    'S2_LOSS_ALPHA_PAIR_MARGIN':    0.3,    # 边界 pair margin 损失权重
    ...
    'FE_EXTRA_DIMS':          36,   # 须与数据集 extra 特征维度一致，否则 Linear(28,256) 与 36 维输入会报 mat1/mat2 shape 错误
    'GRAD_CLIP_NORM':         0.5,
    'REG_LOSS_ALPHA':         0.1,
    'VAL_SPLIT_RATIO':        0.1,
...
def safe_barrier(world_size: int, device: torch.device = None):
    ...
    init_process_group() succeeds. Use explicitly saved _DIST_STORE first to
    avoid _get_default_store()/dist.barrier() path that can deadlock at 10 nodes.
    """
    ...
    # 优先使用 init_distributed 保存的 store，确保首屏 barrier 绝不走 NCCL
    store = _DIST_STORE
    if store is None:
