...
def main():
    ...
    parser.add_argument("--route", required=True, choices=("R5-CensoredFlow",))
    ...
    for step in args.steps:
        ...
        if output.is_file():
            continue
        command = ["sbatch", "--parsable", f"--job-name=r5flowv_{short}_{args.stage}_s{args.seed}_{step}",
    ...
    finalizer_cmd = ["sbatch", "--parsable", f"--dependency={dependency}",
                     f"--job-name=r5flow_finalize_{short}_{args.stage}_retry",
