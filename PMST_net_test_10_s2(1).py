...
CONFIG = {
    ...
    'S1_BEST_CKPT_PATH': '/public/home/putianshu/vis_mlp/checkpoints/exp_1772692533_S1_best_score.pt',

    # ==========================================
    # Stage 2 Phase A 配置
    # ==========================================
    'S2_PHASE_A_STEPS':        30000,
    # [修改] Phase A头部LR轻微提高，有利于在新损失函数下快速适配
    'S2_LR_HEAD_A':            1e-4,
    'S2_LR_FUSION_A':          2e-5,
    'S2_VAL_INTERVAL':         500,
    'S2_BATCH_SIZE':           512,
    ...
    # S2专用二分类权重
    'S2_BINARY_POS_WEIGHT':     1.5,    # ← was 3.0

    # S2专用细粒度类别权重
    'S2_FINE_CLASS_WEIGHT_FOG':   1.5,  # ← was 3.0
    'S2_FINE_CLASS_WEIGHT_MIST':  1.2,  # ← was 2.0
    'S2_FINE_CLASS_WEIGHT_CLEAR': 0.9,  # ← was 0.6 (关键！提高晴天损失权重)

    # S2专用损失系数
    'S2_LOSS_ALPHA_BINARY':    0.7,     # ← was 0.8  (轻微降低二分类权重)
    'S2_LOSS_ALPHA_FINE':      1.0,     # ← was 1.2
    'S2_LOSS_ALPHA_FP':        5.0,     # ← was 1.5  (重大提升：强化假阳性惩罚)
    'S2_LOSS_ALPHA_FOG_BOOST': 0.3,     # ← was 0.5  (降低recall激励)
    'S2_LOSS_ALPHA_MIST_BOOST':0.2,     # ← was 0.3  (降低recall激励)

    # [新增] 晴天边界校准损失 — 核心修复
    # 对晴天样本中 p(fog)+p(mist) > clear_margin 的部分施加平方铰链惩罚。
    # 这是唯一能直接驱动模型在晴天样本上输出低概率的loss项。
    'S2_CLEAR_MARGIN':              0.25,   # 目标：晴天样本fog+mist概率上界
    'S2_LOSS_ALPHA_CLEAR_MARGIN':   3.0,    # 边界损失权重（需要足够大以对抗class weight偏差）
...
class DualBranchLoss(nn.Module):
    """
    Dual-branch loss with an added clear-margin calibration term.

    New term `l_clear_margin`:
        ...
        This is the *only* loss component that directly drives the model to output
        *low* low-visibility probability for clear samples, fixing the root cause
        of precision collapse.

    Backward-compatible: `alpha_clear_margin` defaults to 0.0, so S1 training
    that re-uses this class is unaffected.
    """
    ...
    def forward(self, fine_logits, low_vis_logit, targets):
        ...
        total = (
            ...
            self.cfg.get('alpha_clear_margin', 0.0) * l_clear_margin   # ← NEW
        )
...
def main():
    ...
    def build_s2_loss():
        """
        S2专用损失函数。
        相比build_loss()的三项关键变化：
          1. 更均衡的类别权重(1.5/1.2/0.9)减少recall偏差
          2. alpha_fp=5.0大幅强化假阳性惩罚
          3. alpha_clear_margin=3.0 + clear_margin=0.25：
             对所有p(fog+mist)>0.25的晴天样本施加强制性边界惩罚，
             这是解决精确率崩塌的核心修复。
        """
        return DualBranchLoss(
