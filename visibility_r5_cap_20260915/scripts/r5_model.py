...
class R5Model(nn.Module):
    def __init__(self, cfg: Dict, route: str):
        ...
        self.cap_query = TaskQuery(d, cfg["attention_heads"], cfg["query_ffn_dim"], cfg["dropout"])
        self.continuous_query = TaskQuery(d, cfg["attention_heads"], cfg["query_ffn_dim"], cfg["dropout"])
        self.cap_head = CapHead(d)
        self.continuous_head = DiffusionHead(
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return memory, self.cap_query(memory), self.continuous_query(memory)

    def forward(self, x: torch.Tensor, z_t: Optional[torch.Tensor] = None, timestep: Optional[torch.Tensor] = None):
        memory, h_cap, h_cont = self.encode(x)
        out = {"cap_logit": self.cap_head(h_cap), "memory": memory, "h_cont": h_cont}
        if z_t is not None and timestep is not None:
