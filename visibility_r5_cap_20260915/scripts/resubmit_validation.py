...
def main():
    ...
    parser.add_argument("--route", required=True, choices=("R5-Cap",))
    ...
    for step in args.steps:
        ...
        if output.is_file():
            continue
        command = ["sbatch", "--parsable", f"--job-name=r5capv_{short}_{args.stage}_s{args.seed}_{step}_f20",
    ...
    finalizer_cmd = ["sbatch", "--parsable", f"--dependency={dependency}",
                     f"--job-name=r5cap_finalize_{short}_{args.stage}_retry",
