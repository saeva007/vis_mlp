...
def main():
    ...
    cap, c = float(CFG["cap_value_km"]), CFG["extinction_constant"]
    emin = c / cap
    for stage in ("S1", "S2"):
        root = Path(CFG["data"][stage])
        train = visibility(root / "y_train.npy")
        cont = (train > 0) & (train < cap)
        ...
        contract = {
            ...
            "cap_train_samples": int((train == cap).sum()),
        ...
        strata = [val < 0.5, (val >= 0.5) & (val < 1.0), (val >= 1.0) & (val < cap), val == cap]
