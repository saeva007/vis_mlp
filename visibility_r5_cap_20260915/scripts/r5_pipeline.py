...
def config():
    return json.loads((ROOT / "configs" / "r5_cap.json").read_text(encoding="utf-8"))
...
def z_target(v, stats, cfg):
    emin = cfg["extinction_constant"] / cfg["cap_value_km"]
...
def z_to_visibility(z_scaled, stats, cfg):
    ...
    e = cfg["extinction_constant"] / cfg["cap_value_km"] + torch.exp(raw)
...
def train_loss(raw_model, model, x, v, stats, cfg, schedule):
    cap_value = cfg["cap_value_km"]
    cap_truth = v == cap_value
    cont = (v > 0) & (v < cap_value)
    if raw_model.route == "R5-Gaussian":
        out = model(x)
        cap = F.binary_cross_entropy_with_logits(out["cap_logit"], cap_truth.float())
        if cont.any():
            ...
            nll = interval = out["cap_logit"].sum() * 0
        parts = {"cap": cap, "nll": nll, "interval": interval}
    else:
        b = len(v)
        ...
        if cont.any():
            ...
            diff_loss = boundary = boundary_snr = boundary_weight = out["cap_logit"].sum() * 0
        parts = {"cap": cap, "diffusion": diff_loss, "boundary": boundary,
                 "boundary_snr": boundary_snr, "boundary_weight": boundary_weight}
    if raw_model.route == "R5-Gaussian":
        total = cfg["lambda_cap"] * parts["cap"] + parts["nll"] + cfg["lambda_boundary"] * parts["interval"]
    else:
        total = cfg["lambda_cap"] * parts["cap"] + parts["diffusion"] + cfg["lambda_boundary"] * parts["boundary_snr"]
    return total, parts
...
def prediction_batch(model, x, draws, stats, cfg, schedule, reverse_steps=None):
    out = model(x)
    p_cap = out["cap_logit"].sigmoid().double()
    ...
    probs = torch.stack([
        (1 - p_cap) * cond_probs[:, 0],
        (1 - p_cap) * cond_probs[:, 1],
        p_cap + (1 - p_cap) * cond_probs[:, 2],
    ], 1)
    v_point = p_cap * cfg["cap_value_km"] + (1 - p_cap) * v_samples.mean(1)
    return probs, p_cap, v_point, v_samples, e_samples, out
...
def metrics_from_arrays(v, probs, p_cap, v_point, totals, cfg, route, draws):
    cap_truth = v == cfg["cap_value_km"]
    cap_pred = p_cap >= 0.5
    tp, fp, fn, tn = (int((cap_truth & cap_pred).sum()), int((~cap_truth & cap_pred).sum()),
                      int((cap_truth & ~cap_pred).sum()), int((~cap_truth & ~cap_pred).sum()))
    positive = v > 0
    err = v_point - v
    ...
    loss_excluded = {"n", "crps_sum", "crps_n", "nll_sum", "nll_n",
                     "generated_draws", "generated_e_le_emin", "generated_v_ge_cap"}
    return {
        ...
        "cap": {
            ...
            "recall": tp / (tp + fn) if tp + fn else 0.0, "AUROC": float(roc_auc_score(cap_truth, p_cap)),
            "AP": float(average_precision_score(cap_truth, p_cap)), "Brier": float(np.mean((p_cap - cap_truth) ** 2)),
        },
        "continuous": {
        ...
        "physical_support": {
            ...
            "continuous_V_ge_cap_count": int(totals.get("generated_v_ge_cap", 0)),
        },
...
def evaluate(route, stage, split, checkpoint_path, draws, output_path, save_predictions=False, reverse_steps=None):
    ...
    weights = ckpt.get("ema_model") if route == "R5-Cap" and ckpt.get("ema_model") is not None else ckpt["model"]
    ...
    payload = {"index": [], "v": [], "probs": [], "p_cap": [], "v_point": []}
    totals = {"n": 0.0, "total": 0.0, "cap": 0.0, "nll": 0.0, "diffusion": 0.0, "interval": 0.0,
              "boundary": 0.0, "boundary_snr": 0.0, "boundary_weight": 0.0,
              "crps_sum": 0.0, "crps_n": 0.0, "nll_sum": 0.0, "nll_n": 0.0,
              "generated_draws": 0.0, "generated_e_le_emin": 0.0, "generated_v_ge_cap": 0.0}
    started = time.time()
    effective_reverse_steps = (cfg["diffusion_steps"] if reverse_steps is None else int(reverse_steps)) \
        if route == "R5-Cap" else None
    ...
    for batch_number, (x, v, idx) in enumerate(loader, 1):
        ...
        probs, p_cap, v_point, v_samples, e_samples, out = prediction_batch(
            model, x, draws, stats, cfg, schedule,
            effective_reverse_steps)
        cont = (v > 0) & (v < cfg["cap_value_km"])
        ...
        totals["generated_e_le_emin"] += int((e_samples <= cfg["extinction_constant"] / cfg["cap_value_km"]).sum())
        totals["generated_v_ge_cap"] += int((v_samples >= cfg["cap_value_km"]).sum())
        if cont.any():
        ...
        payload["probs"].append(probs.float().cpu().numpy()); payload["p_cap"].append(p_cap.float().cpu().numpy())
    ...
    if rank == 0:
        ...
        result = metrics_from_arrays(arrays["v"], arrays["probs"], arrays["p_cap"], arrays["v_point"], totals, cfg, route, draws)
...
def quick_validate(model, loader, route, stats, cfg, schedule, draws, seed):
    ...
    arrays = {"v": [], "probs": [], "p_cap": [], "v_point": []}
    sums = {"n": 0.0, "total": 0.0, "cap": 0.0, "nll": 0.0, "diffusion": 0.0, "interval": 0.0,
            "boundary": 0.0, "boundary_snr": 0.0, "boundary_weight": 0.0,
            "crps_sum": 0.0, "crps_n": 0.0, "nll_sum": 0.0, "nll_n": 0.0}
    for x, v, _ in loader:
        x, v = x.cuda(non_blocking=True), v.cuda(non_blocking=True)
        loss, parts = train_loss(model, model, x, v, stats, cfg, schedule)
        probs, p_cap, v_point, v_samples, _, out = prediction_batch(
            model, x, draws, stats, cfg, schedule,
            cfg["quick_reverse_steps"] if route == "R5-Cap" else None)
        cont = (v > 0) & (v < cfg["cap_value_km"])
        ...
        for key, value in (("v", v), ("probs", probs), ("p_cap", p_cap), ("v_point", v_point)):
            arrays[key].append(value.float().cpu().numpy())
    arrays = {k: np.concatenate(v) for k, v in arrays.items()}
    return metrics_from_arrays(arrays["v"], arrays["probs"], arrays["p_cap"], arrays["v_point"], sums, cfg, route, draws)
...
def quick_validate_objective(model, loader, stats, cfg, schedule, seed):
    ...
    model.eval()
    torch.manual_seed(810000 + seed)
    sums = {"samples": 0, "total": 0.0, "cap": 0.0, "nll": 0.0, "diffusion": 0.0, "interval": 0.0,
            "boundary": 0.0, "boundary_snr": 0.0, "boundary_weight": 0.0}
    for x, v, _ in loader:
        x, v = x.cuda(non_blocking=True), v.cuda(non_blocking=True)
        loss, parts = train_loss(model, model, x, v, stats, cfg, schedule)
...
def save_checkpoint(run_dir, step, epoch, model, ema, optimizer, cfg, stats, route, stage):
    ...
    state = {"model": model.state_dict(), "ema_model": ema.state_dict() if ema is not None else None,
             ...
             "normalization": stats, "cap_value_km": cfg["cap_value_km"],
             "extinction_constant": cfg["extinction_constant"], "e_min": cfg["extinction_constant"] / cfg["cap_value_km"],
...
def train(route, stage, seed, init_checkpoint=None):
    ...
    if init_checkpoint:
        ...
        raw_model.load_state_dict(init.get("ema_model") if route == "R5-Cap" and init.get("ema_model") is not None else init["model"])
    model = DDP(raw_model, device_ids=[local], find_unused_parameters=False) if world > 1 else raw_model
    ema = copy.deepcopy(raw_model).eval() if route == "R5-Cap" else None
    if ema is not None:
        for p in ema.parameters(): p.requires_grad_(False)
    head_params, backbone_params = [], []
    for name, p in raw_model.named_parameters():
        (head_params if name.startswith(("cap_", "continuous_")) else backbone_params).append(p)
            ...
            if step % cfg["checkpoint_interval"] == 0:
                if rank == 0:
                    ...
                    if stage == "S2" and step % cfg["async_full_interval"] == 0 and not validator_already_submitted(run_dir, step):
                        ...
                        command = ["sbatch", "--parsable", f"--job-name=r5capv_{stage}_s{seed}_{step}",
    ...
    if rank == 0:
        ...
        if stage == "S1":
            ...
            command += ["--job-name=r5cap_finalize_S2", str(ROOT / "scripts" / "sub_finalize.slurm"), route, stage, str(seed)]
...
def select_s1_handoff(route, seed, submit_s2=False):
    ...
    weight_key = "ema_model" if route == "R5-Cap" and state.get("ema_model") is not None else "model"
    ...
    if submit_s2:
        ...
        try:
            ...
            command = ["sbatch", "--parsable", f"--job-name=r5cap_S2_s{seed}",
...
def describe():
    ...
    for route in ROUTES:
        ...
        modules = {
            ...
            "CapQuery": model.cap_query,
            "CapHead": model.cap_head,
