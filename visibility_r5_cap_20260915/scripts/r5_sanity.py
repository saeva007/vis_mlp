...
model = R5Model(cfg, "R5-Cap").to(device)
model.train()

sums = {"samples": 0, "total": 0.0, "cap": 0.0, "diffusion": 0.0,
...
with torch.inference_mode():
    probs, p_cap, _, v_samples, e_samples, _ = prediction_batch(model, first_x, 4, stats, cfg, schedule, reverse_steps=20)
...
result = {
    ...
    "objective_scale_check": {"samples": count, "means": means, "diffusion_to_snr_boundary_ratio": scale_ratio,
                              ...
                              "lambda_cap": cfg["lambda_cap"], "lambda_boundary": cfg["lambda_boundary"]},
    "loss_finite": all(np.isfinite(value) for value in means.values()),
    "p_cap_range": {"min": float(p_cap.min()), "max": float(p_cap.max()),
                    "valid": bool(((p_cap >= 0) & (p_cap <= 1)).all())},
    "probability_sum_error": float((probs.sum(1) - 1).abs().max()),
    "probabilities_finite": bool(torch.isfinite(probs).all()),
    "physical_support": {
        "E_min": cfg["extinction_constant"] / cfg["cap_value_km"],
        "cap_value_km": cfg["cap_value_km"],
        "min_generated_E": float(e_samples.min()),
        "max_generated_V": float(v_samples.max()),
        "E_le_Emin_count": int((e_samples <= cfg["extinction_constant"] / cfg["cap_value_km"]).sum()),
        "V_ge_cap_count": int((v_samples >= cfg["cap_value_km"]).sum()),
    },
}
(ROOT / "sanity.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
print(json.dumps(result, indent=2))
if (not result["loss_finite"] or not result["p_cap_range"]["valid"] or
        not result["probabilities_finite"] or result["probability_sum_error"] >= 1e-8):
    raise SystemExit(2)
if not result["objective_scale_check"]["within_one_order_of_magnitude"]:
    raise SystemExit(3)
if result["physical_support"]["E_le_Emin_count"] or result["physical_support"]["V_ge_cap_count"]:
    raise SystemExit(4)
