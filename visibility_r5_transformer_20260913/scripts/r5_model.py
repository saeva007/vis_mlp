...
class PointDynamicEncoder(nn.Module):
    """Point weather sequence -> full weather-token sequence."""

    def __init__(self, weather_dim=25, d_model=256, steps=12, layers=6, heads=8, ffn_dim=1024, dropout=0.1):
        super().__init__()
    ...
    def forward(self, weather: torch.Tensor) -> torch.Tensor:
        return self.norm(self.encoder(self.proj(weather) + self.pos[:, : weather.shape[1]]))


class PatchDynamicEncoder(nn.Module):
    """Reserved interface: future patch input must also return [B,T,D] tokens."""
...
class GaussianContinuousHead(nn.Module):
    def __init__(self, d_model=256):
        super().__init__()
...
class DiffusionContinuousHead(nn.Module):
...
class R5Model(nn.Module):
    def __init__(self, cfg: Dict, route: str):
        ...
        d = cfg["d_model"]
        self.dynamic_encoder = PointDynamicEncoder(
            cfg["weather_vars"], d, cfg["window_size"], cfg["transformer_layers"], cfg["attention_heads"],
            cfg["transformer_ffn_dim"], cfg["dropout"]
        )
        ...
        self.continuous_head = GaussianContinuousHead(d) if route == "R5-Gaussian" else DiffusionContinuousHead(
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        weather_tokens = self.dynamic_encoder(dyn[:, :, weather_idx])
