    ...
    if raw_model.route == "R5-Gaussian":
        ...
        if cont.any():
            ...
            for threshold in cfg["interval_thresholds_km"]:
                ...
                boundary_losses.append(F.binary_cross_entropy_with_logits(logits, (v[cont] < threshold).float()))
            interval = torch.stack(boundary_losses).mean()
        else:
            diff_loss = interval = out["gate_logit"].sum() * 0
        parts = {"gate": gate, "diffusion": diff_loss, "interval": interval}
    total = sum(parts.values()) - parts["interval"] + cfg["interval_loss_weight"] * parts["interval"]
    return total, parts
...
def metrics_from_arrays(v, probs, p_gate, v_point, totals, cfg, route, draws):
    ...
    return {
        ...
        "loss": {k: totals[k] / max(totals["n"], 1) for k in totals if k not in ("n", "crps_sum", "crps_n", "nll_sum", "nll_n")},
...
def train(route, stage, seed, init_checkpoint=None):
            ...
            if step % cfg["checkpoint_interval"] == 0:
                if rank == 0:
                    ...
                    if stage == "S2" and step % cfg["async_full_interval"] == 0 and not validator_already_submitted(run_dir, step):
                        ...
                        command = ["sbatch", "--parsable", f"--job-name=r5v_{route.split('-')[1][0]}_{stage}_s{seed}_{step}",
    ...
    if rank == 0:
        ...
        if stage == "S1":
            ...
            command += [str(ROOT / "scripts" / "sub_finalize.slurm"), route, stage, str(seed)]
...
def select_s1_handoff(route, seed, submit_s2=False):
    ...
    if submit_s2:
        ...
        try:
            ...
            command = ["sbatch", "--parsable", f"--job-name=r5_{route.split('-')[1]}_S2_s{seed}",
