...
CONFIG = {
    ...
    # ========== Stage 1 训练配置 ==========
    # [修改1] S1_TOTAL_STEPS: 30000 → 15000，配合Early Stopping节省算力
    # [修改2] S1_VAL_INTERVAL: 1000 → 500，更密集的验证以捕捉最优点
    ...
    # ========== Stage 2 训练配置 ==========
    # [修改3] S2_GRAD_ACCUM: 1 → 2，与S1有效批量对齐（512*2=1024）
    # S2现在分为Phase A（冻结backbone）和Phase B（极低LR全量微调）两个阶段
    'S2_TOTAL_STEPS':         10000,
    'S2_VAL_INTERVAL':        500,
    'S2_BATCH_SIZE':          512,
    'S2_GRAD_ACCUM':          2,    # [修改3] 原为1
    'S2_FOG_RATIO':           0.15,
    'S2_MIST_RATIO':          0.15,
    # Phase A：只训练extra_encoder + heads，backbone完全冻结
    'S2_PHASE_A_STEPS':       5000,
    'S2_LR_HEAD_A':           5e-5,   # Phase A head学习率（backbone已冻结，无需设置）
    # Phase B：全量解冻，极低LR精细微调
    'S2_PHASE_B_STEPS':       5000,
    'S2_LR_BACKBONE_B':       1e-6,   # Phase B backbone LR，原1e-5降低10倍
    'S2_LR_HEAD_B':           5e-6,   # Phase B head LR，原5e-5降低10倍
    'S2_WEIGHT_DECAY':        1e-2,
    # 以下两个旧键保留，用于原始S2 optimizer的向后兼容（实际不再使用）
    'S2_LR_BACKBONE':         1e-5,
    'S2_LR_HEAD':             5e-5,
    ...
    # ========== Early Stopping ==========
    # [修改4] 新增Early Stopping参数
    'S1_ES_PATIENCE':         8,   # S1允许连续多少个val_interval不改善则停止
    'S2_ES_PATIENCE':         6,   # S2早停patience（阶段更短，可以稍紧）
}
...
def copy_to_local(src_path: str, global_rank: int, local_rank: int,
    ...
    # [修复] 拷贝完成后再次同步，必须明确指定 device_ids 防止挂起
    if world_size > 1:
        dist.barrier(device_ids=[local_rank])
...
class ComprehensiveMetrics:
    ...
    def evaluate(self, model, loader, device, rank=0, world_size=1, actual_val_size=None):
        ...
        cur_device = torch.cuda.current_device()  # [修复] 统一获取当前设备
...
# 将 best_ta 广播给所有进程，保证一致性
        if world_size > 1:
            ...
            # [修复] 指定 device_ids 的 barrier，获取当前设备的 index
...
def save_checkpoint(model, path, rank, world_size):
    if rank != 0:
        return
    state = model.module.state_dict() if (world_size > 1 and hasattr(model, 'module')) else model.state_dict()
...
def rewrap_ddp(model, world_size):
    """
    从 DDP wrapper 中提取裸模型（raw module）。
    在重新设置 requires_grad 前必须先调用此函数。
    """
...
def wrap_ddp(raw_model, local_rank, world_size, find_unused=False):
    """
    将裸模型包装为 DDP。
    [修复] 包装前先做 barrier，确保所有 rank 的参数状态一致再建立 reducer。
    """
    if world_size > 1:
        # 确保所有 rank 在同一状态下进入 DDP 构造
        dist.barrier(device_ids=[local_rank])
...
def train_stage(tag, model, tr_ds, val_ds, optimizer, loss_fn, device,
                ...
                patience=10):
    """
    训练一个阶段，支持 Early Stopping。
    所有 DDP rank 通过 broadcast 同步 no_improve_count，保证对称退出。
    """
    ...
    cur_device = torch.cuda.current_device()  # [修复] 缓存当前设备
    ...
    while step < total_steps:
        ...
        if is_last_accum_step:
            ...
            if step % val_int == 0:
                ...
                # 将 no_improve_count 广播给所有 rank，保证对称退出
                if world_size > 1:
                ...
                if patience > 0 and no_improve_count >= patience:
                    ...
                    # [修复] break 前做一次 barrier，确保所有 rank 对称退出训练循环
                    if world_size > 1:
...
def main():
    ...
    # ==========================================
    # [核心修复] 强制进行全局无负载同步！
    # 目的是在进行磁盘 I/O 之前，强制所有卡将 NCCL/RCCL 彻底初始化完成。
    # 消除后续 0卡拷数据、非0卡瞬间进 barrier 导致的硬件时序挂死。
    # ==========================================
    if w_size > 1:
        dist.barrier(device_ids=[l_rank])
    ...
    if g_rank == 0:
        print("\n" + "="*60, flush=True)
        print("[Stage 2] Starting two-phase fine-tune on FE dataset...", flush=True)

    # [核心修复] 先解包旧 DDP，再显式删除旧 DDP wrapper，
    # 释放其在参数上注册的 NCCL reducer hook，防止新 DDP 构造时发生双重 hook 冲突。
    raw_model_s2 = rewrap_ddp(model, w_size)
    del model           # ← 释放 S1 DDP wrapper 及其 NCCL 资源
    torch.cuda.empty_cache()

    if w_size > 1:
        dist.barrier(device_ids=[l_rank])  # 确保所有 rank 完成 del 后再继续

    if os.path.exists(s1_best_score_path):
        loaded = load_checkpoint(raw_model_s2, s1_best_score_path, g_rank, w_size, device)
        if g_rank == 0 and loaded:
            print(
        ...
        if g_rank == 0:
            print(
                f"[Stage 2] WARNING: S1 best_score checkpoint not found at "
                f"{s1_best_score_path}. Continuing with current model weights.",
                flush=True
            )
    ...
    model, opt_s2_a = build_s2_optimizer_phase_a(raw_model_s2)
    loss_fn_s2 = build_loss()

    train_stage(
        tag='S2_PhaseA', model=model, tr_ds=tr_ds_s2, val_ds=val_ds_s2,
        optimizer=opt_s2_a, loss_fn=loss_fn_s2, device=device,
        rank=g_rank, world_size=w_size,
    ...
    # [核心修复] Phase A → Phase B 切换：先解包，显式删除旧 DDP，再创建新 DDP
    raw_model_pb = rewrap_ddp(model, w_size)
    del model           # ← 释放 Phase A DDP wrapper 及其 NCCL 资源
    torch.cuda.empty_cache()
    ...
    if os.path.exists(s2_phase_a_best_path):
        loaded = load_checkpoint(raw_model_pb, s2_phase_a_best_path, g_rank, w_size, device)
        if g_rank == 0 and loaded:
            print(
    ...
    model, opt_s2_b = build_s2_optimizer_phase_b(raw_model_pb)
    loss_fn_s2b = build_loss()

    train_stage(
        tag='S2_PhaseB', model=model, tr_ds=tr_ds_s2, val_ds=val_ds_s2,
        optimizer=opt_s2_b, loss_fn=loss_fn_s2b, device=device,
        rank=g_rank, world_size=w_size,
    ...
    if g_rank == 0:
        print("\n" + "="*60, flush=True)
        print("[Summary] All stages complete.", flush=True)
