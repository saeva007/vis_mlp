...
#S2_DIR = f"/public/home/putianshu/vis_mlp/airport_12h_train_val_test"
CONFIG = {
    # ========== 实验控制配置 ==========
...
class SimpleBaseline(nn.Module):
    """极简3层MLP，用于快速验证特征质量"""
    def __init__(self, input_dim=300+5+1, hidden_dim=256):
        super().__init__()
    ...
    def forward(self, x):
        # 展平时序特征
        dyn_flat = x[:, :300]  # 25变量*12小时
        static = x[:, 300:]    # 静态特征
        x_flat = torch.cat([dyn_flat, static], dim=1)
        logits = self.net(x_flat)
        
        # 为了兼容原训练循环，返回4个值
        return logits, torch.zeros(len(x), 1).to(x.device), \
               torch.zeros(len(x), 1).to(x.device), logits
...
def main():
    ...
    if not CONFIG['SKIP_STAGE1']:
        ...
        s1_train_ds, s1_val_ds, scaler_s1 = load_data_and_scale(
          CONFIG['S1_DATA_DIR'], 
            ...
            reuse_scaler=False, 
            window_size=CONFIG['WINDOW_SIZE'],
            world_size=world_size  # 修复：传递world_size
        )
        ...
        train_with_comprehensive_evaluation(
            ...
            CONFIG['S1_GRAD_ACCUM'], exp_id  # 添加exp_id
        )
    ...
    s2_train_ds, s2_val_ds, scaler_s2 = load_data_and_scale(
        ...
        rank=global_rank, 
        device=device, 
        reuse_scaler=(scaler_s1 is not None),  # 是否复用scaler
        window_size=CONFIG['WINDOW_SIZE'],
        world_size=world_size
    )
    ...
    train_with_comprehensive_evaluation(
        ...
        CONFIG['S2_GRAD_ACCUM'], exp_id  # 添加exp_id
    )
