...
class FogDiagnosticFeatures(nn.Module):
    ...
    def _extract_feature(self, x_seq, feature_name):
        ...
        # [修改点 1] 保留时间维度: [Batch, Window, Val] -> [Batch, Window]
        return x_seq[:, :, idx] 
    ...
    def forward(self, x_dyn_seq):
        ...
        # 堆叠 [Batch, Window, 5] (去掉了原来那个简单的线性 TES，交给网络去学)
...
class GRUWithAttentionEncoder(nn.Module):
    def __init__(self, n_vars, hidden_dim, n_steps=None, dropout=0.2):
        ...
        n_vars: 输入变量数量
        hidden_dim: 隐藏层维度
        n_steps: 时间步长 (为了兼容接口保留此参数，GRU本身是动态的，不强制依赖它)
        """
        super().__init__()
        
        # 1. 特征投影：把输入的物理量映射到高维空间
        ...
        # 2. 双向 GRU：既看过去对现在的影响，也利用后向平滑特征 (Bidirectional)
        # 如果追求严格因果(只看过去)，可设 bidirectional=False，但在离线训练中双向效果通常更好
        self.gru = nn.GRU(
            input_size=hidden_dim, 
            hidden_size=hidden_dim, 
            num_layers=2, 
            batch_first=True, 
            bidirectional=True, # 输出维度会变成 hidden_dim * 2
            dropout=dropout
        )
        ...
        self.attention_net = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(hidden_dim, 1) # 输出每个step的权重分数
        )
        
        # 4. 最终输出投影 (把双向维度降回 hidden_dim)
    ...
    def forward(self, x):
        # x shape: [Batch, Window, Vars] -> 注意：不需要 permute，GRU 原生支持 [B, L, C]
        
        # 1. Embedding
        x_emb = self.embed(x) # [B, W, Hidden]
        ...
        # 在时间维度(dim=1)上做softmax，让权重之和为1
        attn_weights = F.softmax(attn_scores, dim=1) 
        
        # 4. 加权求和 (Context Vector)
        # [B, W, H*2] * [B, W, 1] -> sum(dim=1) -> [B, H*2]
...
class DualStreamPMSTNet(nn.Module):
    def __init__(self, dyn_vars_count=25, window_size=9, 
        ...
        self.temporal_vars_indices = [2, 3, 4, 5, 6, 7] # 降水, 辐射, 风速等流变性强的变量
        temporal_dim = len(self.temporal_vars_indices)
        
        # 实验三核心修改：使用 GRU + Attention 替代 TCN
        # 注意：这里调用的是上面新定义的类
        self.temporal_stream = GRUWithAttentionEncoder(
            n_vars=temporal_dim, 
            hidden_dim=hidden_dim,
            n_steps=window_size # 传入窗口大小供兼容，虽然GRU内部不强制限定长度
        )
    ...
    def forward(self, x):
        ...
        physics_seq = self.fog_diagnostics(x_dyn_seq) # [B, W, 5]
        # 调整维度以适应 Conv1d: [B, C, L]
        ...
        if step % val_interval == 0:
            ...
            if rank == 0:
                ...
                if met['f2_score'] > best_f2:
                    ...
                    save_name = f"pmst_{stage_name.lower()}{win_suffix}best_f2_v3_test1.pth"
                ...
                if met['ts_score'] > best_ts:
                    ...
                    save_name = f"pmst_{stage_name.lower()}{win_suffix}best_ts_v3_test1.pth"
                    save_path = os.path.join(CONFIG['BASE_PATH'], f"model/{save_name}")
                    torch.save(model.module.state_dict() if world_size > 1 else model.state_dict(), save_path)
