...
CONFIG = {
    ...
    # 第一阶段: ERA5
    'S1_DATA_DIR': "/public/home/putianshu/vis_mlp/ml_dataset_pmst_v5_aligned_no_test", 
    ...
    # === 修改 1: 降低正样本采样比例，避免过高虚警 ===
    'S1_POS_RATIO': 0.25,          # 从 0.50 降至 0.25
    
    'S2_TOTAL_STEPS': 15000,       
    'S2_VAL_INTERVAL': 500,
    'S2_BATCH_SIZE': 256,
    'S2_GRAD_ACCUM': 1,
    'S2_POS_RATIO': 0.25,          # 从 0.50 降至 0.25
    
    # === 修改: 评估参数 ===
    'MIN_PRECISION': 0.15,         # 阈值选择时的最低精确率底线
}
...
class RecallFocusedLoss(nn.Module):
    ...
    def __init__(self, gamma_pos=1.0, gamma_neg=3.0, alpha=None, 
        ...
        super().__init__()
        # === 修改: 恢复正常的 Focal Loss 参数 ===
        self.gamma_pos = gamma_pos  # 从 0.5 改回 1.0，减少对简单正样本的过度关注
        ...
        # === 修改: 降低额外惩罚权重 ===
        self.recall_weight = recall_weight # 设为 1.0 即为不启用额外乘法
    ...
    def forward(self, inputs, targets):
        ...
        # 显式召回惩罚 (如果 recall_weight=1.0 则无影响)
...
class FogDiagnosticFeatures(nn.Module):
    ...
    def compute_saturation_proximity(self, rh, dpd):
        rh_current = torch.clamp(rh[:, -1], 0, 100) / 100.0  # 确保RH在[0,100]
        dpd_current = dpd[:, -1]
    ...
    def compute_wind_favorability(self, wspd):
        wspd_current = torch.clamp(wspd[:, -1], min=0)  # 确保非负
    ...
    def compute_stability_index(self, inversion, wspd):
        ...
        wspd_current = torch.clamp(wspd[:, -1], min=0.5)  # === 修复: 风速下限 ===
    ...
    def compute_radiative_cooling_potential(self, sw_rad, lcc, zenith):
        ...
        lcc_current = torch.clamp(lcc[:, -1], 0, 1)  # 确保LCC在[0,1]
    ...
    def compute_vertical_moisture_transport(self, rh2m, rh925):
        rh2m_current = torch.clamp(rh2m[:, -1], 0, 100)  # 确保RH在[0,100]
...
class DualStreamPMSTNet(nn.Module):
    def __init__(self, dyn_vars_count=25, window_size=9, 
        ...
        self.physics_encoder = nn.Sequential(
            nn.Linear(6, 128),           # 6个物理特征
            nn.LayerNorm(128),
            nn.GELU(),
            nn.Dropout(0.2),
            nn.Linear(128, hidden_dim // 4)  # 输出维度
        )
    ...
    def forward(self, x):
        ...
        physics_feat_raw = self.fog_diagnostics(x_dyn_seq)  # [B, 6]
        physics_feat = self.physics_encoder(physics_feat_raw)  # [B, hidden_dim//4]
...
def evaluate_multi_metrics(model, loader, device, return_probs=False, min_precision=0.15):
    """
    修改 3: 优先选择满足最低精确率要求的 F2-Score (倾向于召回) 最高的阈值
    """
    model.eval()
    all_logits = []
    all_targets = []
    ...
    for t in thresholds:
        ...
        # F2 Score: 侧重 Recall (权重是 Precision 的 2 倍)
    ...
    result = {
        'score': best_res['ts'], # 依然用 TS 作为 checkpoint 依据
        ...
        if step % val_interval == 0:
            ...
            if rank == 0:
                ...
                if met['f2_score'] > best_f2:
                    ...
                    save_name = f"pmst_{stage_name.lower()}_best_f2_v3_test.pth"
                ...
                if met['ts_score'] > best_ts:
                    ...
                    save_name = f"pmst_{stage_name.lower()}_best_ts_v3_test.pth"
                    save_path = os.path.join(CONFIG['BASE_PATH'], f"model/{save_name}")
                    torch.save(model.module.state_dict() if world_size > 1 else model.state_dict(), save_path)
...
def main():
    ...
    if global_rank == 0:
        ...
        print("="*60)
        print("  Visibility Forecasting - BALANCED PIPELINE (V3)")
        print("="*60)
        print(f"  Key Adjustments (Soft Landing):")
        print(f"    ✓ Sampling Ratio: 25% Pos (was 50%)")
        print(f"    ✓ Loss: Tamed Alpha & Gamma, Removed Force-Penalty")
        print(f"    ✓ Thresh: Max F2-Score (Recall-Weighted) with Min Precision {CONFIG['MIN_PRECISION']*100:.0f}%")
        print(f"    ✓ Bias Init: Adjusted for 25% Prior")
        print("="*60)
    ...
    if global_rank == 0:
        joblib.dump(scaler_s1, os.path.join(base_path, "scalers/scaler_pmst_balanced_v3_test.pkl"))
    ...
    crit_cls = RecallFocusedLoss(
        gamma_pos=1.0,           # 回调
        gamma_neg=3.0, 
        alpha=[5.0, 2.0, 1.0],   # 降低权重
        recall_weight=1.0        # 移除额外惩罚
    )
