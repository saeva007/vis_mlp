...
CONFIG = {
    ...
    'S2_LOSS_ALPHA_MIST_BOOST':0.5,     # Exp1: was 0.2 (大幅提升，匹配fog_boost级别)
    ...
    'S2_CLEAR_MIST_PENALTY_TH':     0.45,
    'S2_LOSS_ALPHA_CLEAR_MIST':     0.40,
...
def evaluate_per_season(model, loader, device, config, rank=0, temperature=1.0):
    ...
    Thresholds are tuned on softmax(fine / T), matching inference when the same T
    is saved alongside season_thresholds (must match calibrate_temperature).
    """
    if rank != 0:
        return None
    ...
    for s_name, s_months in season_map.items():
        ...
        for f_th in search_grid:
            for m_th in search_grid:
                stats = evaluator._build_full_metrics(s_probs, s_targets, f_th, m_th)
                if stats['Fog_P'] >= 0.05 and stats['Mist_P'] >= 0.05 and stats['Clear_R'] >= 0.85:
...
def main():
    ...
    if g_rank == 0 and season_thresholds:
        ...
        best_mode_name, (best_mode_probs, best_mode_preds, best_mode_stats) = max(
            mode_candidates.items(), key=lambda item: item[1][2]['score']
        )
