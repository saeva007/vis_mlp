...
class DualBranchLoss(nn.Module):
    def __init__(self, 
                 ...
                 alpha_fp=1.5, 
                 alpha_fog_boost=0.6,    # Fog增强系数
                 alpha_mist_boost=0.6,   # Mist增强系数（与Fog相等）
                 fp_threshold=0.6,
        ...
        self.register_buffer('fine_class_weight', torch.tensor([2.0, 2.0, 1.0]))  # Fog, Mist, Clear
    ...
    def forward(self, final_logits, low_vis_logit, fine_logits, targets):
        ...
        return total_loss, {
            ...
            'fog_boost': fog_boost.item(),      # 新增监控
            'mist_boost': mist_boost.item(),    # 新增监控
...
class ComprehensiveMetrics:
    ...
    def search_optimal_thresholds(self, all_probs, all_targets, rank=0):
        ...
        阶段3：宽松模式（只保证Clear）
        """
        fog_thresholds = np.arange(0.10, 0.90, 0.05)
        mist_thresholds = np.arange(0.10, 0.90, 0.05)
        ...
        for fog_th in fog_thresholds:
            for mist_th in mist_thresholds:
                ...
                # 平衡的综合得分
                score = 0.45 * fog_m['f2'] + 0.40 * mist_m['f2'] + 0.15 * clear_m['f2']
        ...
        if best_score < 0:
            ...
            for fog_th in fog_thresholds:
                for mist_th in mist_thresholds:
                    ...
                    # 只检查Fog和Clear约束
                    if clear_m['recall'] < 0.90:  # 放宽到90%
                        continue
                    if fog_m['precision'] < 0.18:  # 稍微放宽
                        continue
                    if fog_m['recall'] < 0.45:     # 稍微放宽
                        continue
                    
                    # Fog优先的得分
                    score = 0.60 * fog_m['f2'] + 0.30 * mist_m['f2'] + 0.10 * clear_m['f2']
        ...
        if best_score < 0:
            ...
            for fog_th in fog_thresholds:
                for mist_th in mist_thresholds:
                    ...
                    # 只保证Clear准确率
                    if clear_m['recall'] < 0.90:
                        continue
                    
                    # 平衡得分
                    score = 0.50 * fog_m['f2'] + 0.35 * mist_m['f2'] + 0.15 * clear_m['f2']
    ...
    def evaluate_comprehensive(self, model, val_loader, device, search_threshold=True, rank=0):
        ...
        # 1. 计算各类别指标 (包含 precision, recall, f2, tp, fp, fn)
        ...
        # 3. 综合得分
        composite_score = 0.45 * fog_metrics['f2'] + 0.40 * mist_metrics['f2'] + 0.15 * clear_metrics['f2']
        ...
        # 返回完整的指标字典
        return {
            'fog': fog_metrics,                          # ✓ 包含 precision, recall, f2, tp, fp, fn
            'mist': mist_metrics,                        # ✓ 包含 precision, recall, f2, tp, fp, fn
            'clear': clear_metrics,                      # ✓ 包含 precision, recall, f2, tp, fp, fn
            ...
            if rank == 0:
                ...
                if metrics['composite_score'] > best_scores['composite']:
                    ...
                    save_path = os.path.join(CONFIG['BASE_PATH'], 
                        f"model/pmst_{stage_name.lower()}{win_suffix}best_composite_v6.pth")
                    ...
                    threshold_path = os.path.join(CONFIG['BASE_PATH'],
                        f"model/pmst_{stage_name.lower()}{win_suffix}best_composite_thresholds_v6.pkl")
                ...
                if metrics['fog']['f2'] > best_scores['fog_f2']:
                    ...
                    save_path = os.path.join(CONFIG['BASE_PATH'],
                        f"model/pmst_{stage_name.lower()}{win_suffix}best_fog_f2_v6.pth")
                    torch.save(model.module.state_dict() if world_size > 1 else model.state_dict(), save_path)
                    threshold_path = os.path.join(CONFIG['BASE_PATH'],
                        f"model/pmst_{stage_name.lower()}{win_suffix}best_fog_f2_thresholds_v6.pkl")
                    joblib.dump(evaluator.best_thresholds, threshold_path)
                ...
                if metrics['mist']['f2'] > best_scores['mist_f2']:
                    ...
                    save_path = os.path.join(CONFIG['BASE_PATH'],
                        f"model/pmst_{stage_name.lower()}{win_suffix}best_mist_f2_v6.pth")
                    torch.save(model.module.state_dict() if world_size > 1 else model.state_dict(), save_path)
                    threshold_path = os.path.join(CONFIG['BASE_PATH'],
                        f"model/pmst_{stage_name.lower()}{win_suffix}best_mist_f2_thresholds_v6.pkl")
                    joblib.dump(evaluator.best_thresholds, threshold_path)
                ...
                if metrics['low_vis_metrics']['ts'] > best_scores['low_vis_ts']:
                    ...
                    save_path = os.path.join(CONFIG['BASE_PATH'],
                        f"model/pmst_{stage_name.lower()}{win_suffix}best_low_vis_ts_v6.pth")
                    torch.save(model.module.state_dict() if world_size > 1 else model.state_dict(), save_path)
                    threshold_path = os.path.join(CONFIG['BASE_PATH'],
                        f"model/pmst_{stage_name.lower()}{win_suffix}best_low_vis_ts_thresholds_v6.pkl")
                    joblib.dump(evaluator.best_thresholds, threshold_path)
...
def main():
    ...
    if global_rank == 0:
        scaler_name = f"scaler_pmst_v6_{CONFIG['WINDOW_SIZE']}h.pkl"
    ...
    dual_loss = DualBranchLoss(
        ...
        alpha_consistency=0.5,
        alpha_fp=1.5,
        alpha_fog_boost=0.6,     # Fog增强
        alpha_mist_boost=0.6,    # Mist增强（相等）
