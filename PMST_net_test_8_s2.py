...
CONFIG = {
    ...
    # ========== Stage 2 训练配置 ==========
    'S2_TOTAL_STEPS':         35000,
    'S2_VAL_INTERVAL':        500,
    'S2_BATCH_SIZE':          512,
    'S2_GRAD_ACCUM':          2,
    'S2_FOG_RATIO':           0.2,
    'S2_MIST_RATIO':          0.2,
    'S2_PHASE_A_STEPS':       20000,
    'S2_LR_HEAD_A':           5e-5,
    'S2_PHASE_B_STEPS':       15000,
    'S2_LR_BACKBONE_B':       1e-6,
    'S2_LR_HEAD_B':           5e-6,
...
def _enforce_nccl_shm_disable():
    """
    Safety net: ensure NCCL_SHM_DISABLE=1 is set in the process environment
    before any NCCL/RCCL code path is triggered.
    """
...
def safe_barrier(world_size: int, device: torch.device = None):
    ...
    FIX: Added torch.cuda.synchronize() after the all_reduce to guarantee
    the GPU has fully completed the operation before any rank proceeds.
    Without this, fast ranks may advance past the barrier before slower
    ranks have committed their side of the collective, causing subtle
    race conditions in subsequent NFS/file operations.
    """
    if world_size > 1:
...
def copy_to_local(src_path: str, global_rank: int, local_rank: int,
    ...
    """
    Copy a file from NFS to local /tmp on each node (gated by local_rank==0).
    All world_size ranks participate in the barriers.
    """
    target_dir = "/tmp"
    if exp_id is None:
        exp_id = "default_exp"
    ...
    # Barrier 1: ensure all ranks are ready before any copy starts
    ...
    # Barrier 2: wait for all local_rank==0 nodes to finish copying
    safe_barrier(world_size, device)
...
def init_distributed():
    ...
    torchrun --rdzv_backend=static creates a TCPStore server on the
    --rdzv_endpoint port (RDZV_PORT) before the Python training script
    starts. torchrun then sets MASTER_PORT=RDZV_PORT in every worker
    process's environment. When the training script called
    dist.TCPStore(port=MASTER_PORT, is_master=(rank==0)), rank 0 tried
    to bind a SECOND server on the already-occupied RDZV_PORT, causing
    "errno 98: Address already in use". Rank 0 exited, and all other
    ranks received "Connection reset by peer" when fetching the NCCL
    unique ID from rank 0's now-dead store.

    FIX:
    The SLURM script now exports DIST_STORE_PORT = RDZV_PORT + 1.
    torchrun does not override custom environment variables, so
    DIST_STORE_PORT arrives in every worker process with the correct
    value. This function reads DIST_STORE_PORT (not MASTER_PORT) for
    its TCPStore, completely eliminating the port conflict.

    FALLBACK:
    If DIST_STORE_PORT is not set (e.g., single-node interactive run),
    the function falls back to MASTER_PORT + 1 so it is always distinct
    from whatever port torchrun may be using.
    """
    ...
    if world_size > 1 and not dist.is_initialized():
        ...
        # ---------------------------------------------------------------
        # CRITICAL: Read the training-store port from DIST_STORE_PORT,
        # not MASTER_PORT. MASTER_PORT is set by torchrun to the rdzv
        # endpoint port, which is already bound by torchrun's own
        # TCPStore server. DIST_STORE_PORT is a separate port exported
        # by the SLURM script and is never touched by torchrun.
        #
        # Fallback: if running outside SLURM (e.g., local debug with
        # torchrun only), use MASTER_PORT + 1 as a safe default.
        # ---------------------------------------------------------------
        ...
        # Rank 0 is the TCPStore server; all other ranks are clients.
        # This store is on a port that torchrun has never touched.
        store = dist.TCPStore(
...
class StratifiedBalancedBatchSampler(Sampler):
    """
    FIX: The sampler now stores and yields indices into the SUBSET (positions
    within the PMSTDataset), not into the original full array.

    The dataset is already subset-aware via self.indices[] mapping.
    The sampler must generate positions 0..len(dataset)-1, which
    __getitem__ receives and then maps to original-array rows via
    self.indices[idx].  Previously the sampler was yielding original-array
    indices, causing __getitem__ to double-index and either crash or
    ...
    def __init__(self, dataset, batch_size, fog_ratio=0.2, mist_ratio=0.2,
        ...
        # y_cls on the dataset is already the SUBSET labels (length = len(dataset))
        # So positions 0..len(dataset)-1 are valid indices for __getitem__.
        y = dataset.y_cls.numpy()  # Already subset-aligned
        ...
        # Build per-class position lists (positions within the subset, 0..N-1)
        ...
        # Shard each class across ranks
        for k in self.pos_indices:
            if len(self.pos_indices[k]) == 0:
                # Safety fallback: use all positions of any class
...
class PMSTDataset(Dataset):
    """
    FIX: __getitem__ receives a SUBSET-LOCAL position idx (0..len(self)-1).
    It maps that to the original-array row via self.orig_indices[idx].
    self.y_cls and self.y_reg are already aligned to the subset, so they
    are indexed directly by idx (the subset-local position).
    """
    def __init__(self, X_path, y_cls, y_raw, scaler=None,
                 window_size=12, use_fe=True, indices=None):
        self.X_path  = X_path
        # Store the mapping from subset-position -> original-array row
        self.orig_indices = np.asarray(indices) if indices is not None else np.arange(len(y_cls))

        # y_cls and y_reg are SUBSET-aligned: index 0..len(orig_indices)-1
        ...
        # X is loaded lazily per worker (mmap-safe)
        self.X = None
    ...
    def __getitem__(self, idx):
        """
        idx is a subset-local position (0..len(self)-1), as yielded by
        StratifiedBalancedBatchSampler.
        """
        if self.X is None:
            self.X = np.load(self.X_path, mmap_mode='r')

        # Map subset-local position to original-array row index
        ...
        # y_cls/y_reg are already subset-aligned, index directly by idx
...
def load_data(data_dir, scaler=None, rank=0, local_rank=0, device=None,
    ...
    """
    Load dataset from data_dir, fitting/loading a RobustScaler as needed.
    All safe_barrier calls receive the device explicitly so RCCL
    communicator initialization is forced on the correct GPU.
    """
    ...
    if scaler is None and not reuse_scaler:
        # Barrier: ensure all ranks synchronize before rank-0 scaler work
        safe_barrier(world_size, device)
        ...
        # Barrier: wait for rank-0 to finish writing scaler before all ranks load it
        safe_barrier(world_size, device)

        # Poll for scaler file (NFS may have a short propagation delay)
        wait_time = 0
...
def load_checkpoint(model, path, rank, world_size, device):
    """
    FIX: The original code raised FileNotFoundError inside 'if rank == 0:'
    and then called safe_barrier() unconditionally. If rank 0 raised the
    exception, all other ranks would reach the barrier and hang forever.

    Fixed by checking path existence on ALL ranks before the barrier, so
    any rank can detect the missing file and the error propagates cleanly
    without leaving other ranks stuck at the barrier.
    """
    path_exists = os.path.exists(path)

    # Broadcast existence flag from rank 0 to all ranks so the decision
    # is consistent across the entire job.
    if world_size > 1:
    ...
    # All ranks synchronize before loading to ensure the file is fully
    # written and visible on NFS before any rank opens it.
    safe_barrier(world_size, device)
...
def train_stage(tag, model, tr_ds, val_ds, optimizer, loss_fn, device,
                ...
                patience=10):
    """
    FIX: DataLoader for training now uses persistent_workers=True and
    worker_init_fn to reset the per-worker mmap handle. This prevents
    the DataLoader from hanging when worker processes try to share the
    mmap'd file handle across forks.

    FIX: num_workers reduced to a safe value (2) to avoid overloading
    NFS/local-tmp I/O with 40 ranks * 4 workers = 160 concurrent readers.
    """
...
def main():
    ...
    # Force a real collective immediately after init to confirm all 40 ranks
    # can communicate. If this passes, the NCCL ring is healthy. If it hangs,
    # the issue is network/RCCL, not a code bug — check NCCL_SOCKET_IFNAME
    # routing and ib0 connectivity between all node pairs.
    ...
    def build_s2_optimizer_phase_a(raw_model):
        ...
        for name, param in raw_model.named_parameters():
            ...
            if module_name in HEAD_PARAM_NAMES or module_name == 'extra_encoder':
        ...
        if g_rank == 0:
            print(
                f"[S2-PhaseA-Optim] Frozen: backbone. "
                f"Trainable modules: extra_encoder + heads",
                flush=True
            )
            print(
                f"[S2-PhaseA-Optim] Trainable param tensors: {len(trainable_params)}",
                flush=True
            )
            print(f"[S2-PhaseA-Optim] LR={CONFIG['S2_LR_HEAD_A']}", flush=True)

        # find_unused_parameters=True is required when backbone is frozen.
        # Without it, DDP will error or hang because frozen parameters are not
        # used in the forward pass and therefore have no gradients in the bucket.
        new_model = wrap_ddp(raw_model, l_rank, w_size, find_unused=True)

        optimizer = optim.AdamW(
            trainable_params,
            lr=CONFIG['S2_LR_HEAD_A'],
    ...
    def build_s2_optimizer_phase_b(raw_model):
        ...
        if g_rank == 0:
            ...
            print(
                f"[S2-PhaseB-Optim] LR backbone={CONFIG['S2_LR_BACKBONE_B']}, "
                f"LR head={CONFIG['S2_LR_HEAD_B']}",
                flush=True
            )

        # find_unused=False is correct here: all params are trainable
    ...
    # ==========================================
    # Stage 2 Phase A：冻结 backbone，只训练 extra_encoder + heads
    # ==========================================
    if g_rank == 0:
        print("\n" + "-"*60, flush=True)
        print(
            f"[Stage 2 / Phase A] Backbone FROZEN. "
            f"Steps={CONFIG['S2_PHASE_A_STEPS']}",
            flush=True
        )

    model_pa, opt_pa = build_s2_optimizer_phase_a(raw_model)
    loss_fn_pa = build_loss()
    ...
    # ==========================================
    # Stage 2 Phase B：解冻全部参数，极低 LR 全量微调
    # ==========================================
    if g_rank == 0:
        print("\n" + "-"*60, flush=True)
        print(
            f"[Stage 2 / Phase B] Backbone UNFROZEN with very low LR. "
            f"Steps={CONFIG['S2_PHASE_B_STEPS']}",
            flush=True
        )

    model_pb, opt_pb = build_s2_optimizer_phase_b(raw_model_pb)
    loss_fn_pb = build_loss()
