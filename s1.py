...
# ==========================================
# 0. 全局配置 — 使用 PMST_s1_data_pm10.ipynb 构建的数据集
# ==========================================
TARGET_WINDOW_SIZE = 12
BASE_PATH = "/public/home/putianshu/vis_mlp"
# 数据集目录（PM10+PM2.5 时改为 ..._pm10_pm25；脚本会按 X 列宽自动识别 dyn=26/27）
...
CONFIG = {
    ...
    # - 为避免未来数据集重建时维度变化导致静默接错，这里仅作为默认值，
    #   训练时会从 X_train.npy 自动推断并强制校验。
    # After moving pm10 into dyn, FE extra dims should be 32+4 (=36).
    'FE_EXTRA_DIMS':          36,
    'GRAD_CLIP_NORM':         0.5,
    'REG_LOSS_ALPHA':         0.1,
...
def safe_barrier(world_size: int, device: torch.device = None):
    ...
    在当前 GPU 上做强制计算 + dummy all_reduce + cuda.synchronize，
    保证集体通信与 GPU 完成顺序一致。
    【核心修复】: 在 AMD 显卡上，改用 ones 并执行加法运算，强制触发 
    Compute Kernel，彻底唤醒 GPU Context，防止 Lazy Init 造成 RCCL 死锁。
    """
    if world_size <= 1 or not dist.is_available() or not dist.is_initialized():
        return
    if device is not None:
        # 强制申请显存并进行实际计算运算，唤醒 GPU
...
def load_data(data_dir, scaler=None, rank=0, local_rank=0, device=None,
    ...
    """
    训练/验证与 PMST_s1_data_pm10.ipynb 一致：X_train/y_train + X_val/y_val。
    ...
    if not os.path.isfile(path_X_va) or not os.path.isfile(path_y_va):
        raise FileNotFoundError(
            ...
            "Run PMST_s1_data_pm10.ipynb (or equivalent) so val split matches monthly+gap design."
        )
    ...
    scaler_path = os.path.join(
        ...
        f'robust_scaler_w{win_size}_dyn{dyn_vars_count}_s1_pm10.pkl'
    )
...
def train_stage(tag, model, tr_ds, val_ds, optimizer, loss_fn, device,
    ...
    # 卡住顺序：无 "RCCL warmup passed" → 挂在本 warmup；无 "First batch received" → DataLoader；无 "First step done" → 首次梯度 all_reduce。
    if world_size > 1:
